...
func (h *DownloaderHandlers) AuthLoginPageHandler(ctx *fasthttp.RequestCtx) {
	...
	cssPaths, err := paths.AuthCssPaths(h.assetFolders.Css())
	if err != nil {
		nfasthttp.WriteErrorx(ctx, err)
		return
	}
	...
	// Load template
	tmpl, err := h.loadPageTemplate(pages.AuthLoginPage.FileName())
	if err != nil {
	...
	// Execute template
	if err := tmpl.ExecuteTemplate(ctx, pages.AuthLoginPage.Key(), pageData); err != nil {
		nfasthttp.WriteErrorx(ctx, errInternal(err))
		return
	}
}
