...
func ResolveUser(ctx *fasthttp.RequestCtx) *dauth.AuthContext {
...
func ResolveUserOrAnonym(ctx *fasthttp.RequestCtx) dauth.AuthContext {
	...
	return dauth.AuthContextAnonymous(anonSessionID)
}
...
func EnsureUser(ctx *fasthttp.RequestCtx) (dauth.AuthContext, error) {
	userCtx := ResolveUser(ctx)

	if userCtx == nil && !nfasthttp.IsForwardedHTTPS(ctx) {
		return dauth.AuthContext{}, errorx.NewHTTPMessage("HTTPS is required for authentication", fasthttp.StatusBadRequest)
	}

	if userCtx == nil {
		return dauth.AuthContext{}, ierrors.ErrUnauthorized
	}
...
func ResolveUserOrFallback(ctx *fasthttp.RequestCtx, appMode dtypes.AppMode) (dauth.AuthContext, error) {
