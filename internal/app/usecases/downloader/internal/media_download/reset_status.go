...
func (uc *MediaDownload) ResetStatus(ctx context.Context) error {
	...
	err := uc.downloadRepo().UpdateStatus(ctx, statusesToNew, dtypes.MediaDownloadStatusNew)
	if err != nil {
	...
	err = uc.downloadRepo().UpdateStatus(ctx, statusesToDone, dtypes.MediaDownloadStatusDone)
	if err != nil {
