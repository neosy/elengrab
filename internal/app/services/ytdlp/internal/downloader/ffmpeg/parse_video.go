...
func (ffi *Info) parseVideo(line string, srcVideoInfo *ddownload.VideoInfo) *ddownload.VideoInfo {
	if line == "" {
		return nil
	}
	...
	for i, value := range strings.Split(line, ", ") {
		...
		if !infoSetter.resolution && ffi.resolutionRe.MatchString(fields[0]) {
			res := fields[0]
