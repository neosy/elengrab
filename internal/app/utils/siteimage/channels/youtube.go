...
func FetchYouTubeImages(ctx context.Context, channelURL string, opts ...FetchOption) ([]dtypes.ImageData, error) {
	fetchOptions := types.NewFetchOptions(opts...)
	...
	for _, src := range sources {
		image, err := fetch.FetchImage(ctx, src.URL, opts...)
