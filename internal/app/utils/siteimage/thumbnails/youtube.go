...
func fetchYouTubeShortImage(ctx context.Context, url string, opts ...FetchOption) (*dtypes.ImageData, error) {
	...
	for _, url := range urls {
		...
		imageData, err := fetch.FetchImage(ctx, imageURL, opts...)
		if imageData != nil {
...
func fetchYouTubeImage(ctx context.Context, mediaURL string, opts ...FetchOption) (*dtypes.ImageData, error) {
...
func FetchYouTubeImages(
	...
	imageData, _ := fetchYouTubeImage(ctx, mediaURL, opts...)
	if imageData != nil {
		images = append(images, imageData)
	}

	imageData, _ = fetchYouTubeShortImage(ctx, mediaURL, opts...)
	if imageData != nil {
		images = append(images, imageData)
	}
