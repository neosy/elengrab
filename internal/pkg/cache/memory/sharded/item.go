...
// Valid reports whether the item is still valid.
// Returns true if:
//   - the expiration time is zero (never expires), or
//   - the current time is before or equal to the expiration time
//     (the item is valid at the exact moment of expiration).
