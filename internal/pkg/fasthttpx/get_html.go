...
// GetHTMLWithCookie performs an HTTP GET request using fasthttp and returns
// the response body as a safe-to-use byte slice.
func GetHTMLWithCookies(url string, cookies []string, opts ...ClientOption) ([]byte, error) {
	...
	for _, cookie := range cookies {
		...
		req.Header.SetCookie(parts[0], parts[1])
	}
...
func GetHTML(
	...
	return GetHTMLWithCookies(url, nil, opts...)
}
